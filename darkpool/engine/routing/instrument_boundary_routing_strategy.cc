...
InstrumentBoundaryRoutingStrategy::InstrumentBoundaryRoutingStrategy(std::vector<std::string> ordered_partitions, bool concurrent)
: ordered_partitions(ordered_partitions), concurrent(concurrent)
{
    create_engines();
}

InstrumentBoundaryRoutingStrategy::InstrumentBoundaryRoutingStrategy(int num_partitions, bool concurrent) : concurrent(concurrent) {    
...
std::vector<MatchingEnginePtr>& InstrumentBoundaryRoutingStrategy::get_engines() {
    return engines;
}

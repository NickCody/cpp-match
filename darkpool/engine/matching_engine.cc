...
namespace darkpool {
  ...
  MatchingEngine::~MatchingEngine() {
    ...
    order_book_sizes.write_max_by_second(
        "Total (average) orders managed (buys/sells) by each order book in this engine (per second bin):");
  }

  bool MatchingEngine::is_concurrent() { return concurrent; }
  ...
  void MatchingEngine::process_queued_orders() {
    ...
    while (true) {
      ...
      // std::cout << "MatchingEngine(" << name << ") processing " << internal_queue.size() << " orders." <<
      // std::endl;
  ...
  void MatchingEngine::terminate_processing() { terminate.store(true); }
  ...
  void MatchingEngine::print_trade(const Order& o1, const Order& o2, double price, int trade_quantity) {
    ...
    std::cout << "TRADE " << o1.instrument << " " << o1.order_id << " " << o2.order_id << " " << trade_quantity << " "
              << price << std::endl;
  }

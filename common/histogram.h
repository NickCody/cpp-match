...
class Histogram {
    ...
    public:
        ...
        // record_value
        //
        void record_value(const T& v) {
            measurements.push_back(Measurement(get_current_milliseconds(), v));
        }
        
        void record_value(std::chrono::milliseconds ms, const T& v) {
            measurements.push_back(ms, v);
        }
        
        // Per-second measurements
        //
        void write_sum_by_second(const std::string& notice) {
            write_sum_by_second(notice, name + PER_SECOND_EXTENSION);
        }
        ...
        void write_max_by_second(const std::string& notice) {
            write_max_by_second(notice, name + PER_SECOND_EXTENSION);
        }
        ...
        void write_percentiles() {            
            write_percentiles(name + HISTOGRAM_EXTENSION);
        }
        ...
        PercentileArray calculate_percentiles(const std::vector<Measurement>& m) {
            ...
            ValueArray values;
            std::transform(m.begin(), m.end(), std::back_inserter(values), [](const Measurement& m) { return std::get<1>(m); });
        ...
        void write_percentiles(std::ostream& o, const std::vector<Measurement>& m, const PercentileArray& percentiles) {
            ...
            for ( const Percentile& p : percentiles) {
                o << "  percentile " << std::setprecision(3) << std::get<0>(p) << " = " << std::setprecision(3) << std::get<1>(p)<< std::endl;
            }
        }
};
